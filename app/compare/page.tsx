...
import { creditCards, getCardById, type CreditCard } from "@/lib/data/cards"
...
const comparisonFeatures = [
  { key: "annualFee", label: "Annual Fee", format: (v: number) => v === 0 ? "FREE" : `₹${v.toLocaleString()}` },
  { key: "joiningFee", label: "Joining Fee", format: (v: number) => v === 0 ? "FREE" : `₹${v.toLocaleString()}` },
  { key: "rewardsRate", label: "Rewards Rate", format: (v: number, card: CreditCard) => `${card.rewards.rate}% ${card.rewards.type}` },
  { key: "welcomeBonus", label: "Welcome Bonus", format: (_: string | null, card: CreditCard) => card.welcomeBonus || "None" },
...
export default function ComparePage() {
  ...
  const selectedCards = useMemo(() => 
    selectedCardIds.map(id => getCardById(id)).filter(Boolean) as CreditCard[],
    [selectedCardIds]
  )
              ...
              return (
                ...
                >
                  ...
                  {selectedCards.map((card) => {
                    ...
                    const value = feature.format(card[feature.key as keyof CreditCard] as never, card)
